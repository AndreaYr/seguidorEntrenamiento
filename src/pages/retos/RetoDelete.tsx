    ...
    <DashboardLayout>
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 mb-6">
          ...
          <p><span className="font-semibold">Nombre:</span> {reto.nombre}</p>
        </div>
        <div className="flex gap-4">
          <button onClick={handleDelete} className="px-6 py-2 bg-red-600 text-white rounded">🗑️ Sí, Eliminar</button>
          <button onClick={() => navigate('/retos')} className="px-6 py-2 bg-gray-500 text-white rounded">Cancelar</button>
        </div>
      </div>
    </DashboardLayout>
  );
